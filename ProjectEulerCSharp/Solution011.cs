...
namespace ProjectEulerCSharp
{
    ...
    public class Solution011
    {
        ...
        private IEnumerable<long> Products()
        {
            var s = new[]
            {
            ...
            for (int i = 0; i < 20; i++)
            {
                for (int j = 0; j < 20; j++)
                {
                    var inFromRight = j <= 16;
                    var inFromLeft = j >= 3;
                    var upFromBottom = i <= 16;
                    if (upFromBottom)
                    {//vertical
                        yield return Enumerable.Range(0, 4).Select(offset => s[i + offset][j]).Product();
                    }
                    if (inFromRight)
                    {
                        yield return Enumerable.Range(0, 4).Select(offset => s[i][j + offset]).Product();
                        //horizontal
                    }
                    if (upFromBottom && inFromRight)
                    {
                        yield return Enumerable.Range(0, 4).Select(offset => s[i + offset][j + offset]).Product();
                        //UL => DR diag
                    }
                    if (upFromBottom && inFromLeft)
                    {
                        //UR => DL
                        yield return Enumerable.Range(0, 4).Select(offset => s[i + offset][j - offset]).Product();
                    }
